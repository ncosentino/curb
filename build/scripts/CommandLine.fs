...
type Arguments =
    ...
    | [<CliPrefix(CliPrefix.None);SubCommand>] Compare
    | [<CliPrefix(CliPrefix.None);SubCommand>] Options
    ...
    | [<CliPrefix(CliPrefix.None);SubCommand>] VerifyExpectationsIde0055
...
with
    interface IArgParserTemplate with
        member this.Usage =
            ...
            | Compare -> "times curb, csharpier and dotnet format over a directory of repos (--corpus <dir>)"
            | Options -> "generates docs/reference/**/*.md by running Curb against each option's snippet"
            ...
            | Corpus _ -> "path to a checkout (or a directory of checkouts for compare)"
            ...
            | CleanupSmoketest -> "prove curb cleanup fixes what a build reported, and only that"
            ...
            | VerifyExpectationsIde0055 -> "prove every expectation in the test suite reports no IDE0055 under a real, analyzer-driven dotnet build — a fixed point of dotnet format whitespace is not the same claim"
