...
internal static class Fingerprint
{
	...
	/// of Curb doing the binding, and the version is already in the cache header — so this fingerprint
	/// <em>cannot</em> miss a change that would alter formatting. It can move when a key Curb never reads
	...
	internal static UInt128 OfOptions(FileConfiguration configuration)
	{
		...
		try
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				// allocate. Every RID Curb publishes is little-endian, and the cache never travels
