...
{
	/// <summary>
	/// <c>dotnet format style</c>'s territory. Curb reports these and points there; it does not
	/// attempt them.
	/// </summary>
	DotnetFormatStyle,

	/// <summary>Curb's formatter already satisfies it from syntax alone, in the same pass as IDE0055.</summary>
	Formatting,

	/// <summary><c>curb cleanup</c> fixes it, from a diagnostic a build already reported.</summary>
	Cleanup,

	/// <summary>Curb will never fix it. <see cref="RuleEntry.Refusal"/> carries the reason.</summary>
	Never,
}
...
/// <remarks>
/// The distinction matters to <c>--forward</c>. "Curb will not do this" and "nobody should do this
/// unattended" are different claims, and collapsing them into one would have Curb withhold a rule from
/// another tool that does it perfectly well — on the strength of a constraint that is Curb's own.
/// </remarks>
public enum RefusalScope
{
	/// <summary>
	/// Curb will not, but another tool legitimately can — usually because the fix deletes a declaration,
...
/// <summary>One code style rule, and Curb's position on it.</summary>
...
/// <param name="Refusal">Why Curb will not fix it, when <paramref name="Owner"/> is <see cref="RuleOwner.Never"/>.</param>
/// <param name="Scope">Whether the refusal is Curb's alone or applies to any tool.</param>
public readonly record struct RuleEntry(
	...
	TokenDelta Delta,
	string? Refusal = null,
	RefusalScope Scope = RefusalScope.Curb);

/// <summary>
/// Every code style rule the .NET SDK can report, and whether Curb fixes it.
/// </summary>
/// <remarks>
/// <para>
...
/// a message Curb prints matches the one the build printed.
/// </para>
/// <para>
/// The point of naming all 116 rather than only the ones Curb touches is the same honesty
/// <see cref="OptionCatalog"/> exists for: a user looking at a diagnostic wants to know whether Curb
...
public static class RuleCatalog
{
	...
	[
		...
		new("IDE0034", "Simplify 'default' expression",                                    RuleOwner.Cleanup, TokenDelta.Dropped),
		...
		new("IDE0071", "Simplify interpolation",                                           RuleOwner.Cleanup, TokenDelta.Dropped),
		...
		new("IDE0160", "Convert to block scoped namespace",                                RuleOwner.Never, TokenDelta.None, "Curb converts to a file-scoped namespace and never back; removing braces can change what a name resolves to."),
		...
		new("IDE0240", "Remove redundant nullable directive",                              RuleOwner.Cleanup, TokenDelta.Dropped),
		new("IDE0241", "Remove unnecessary nullable directive",                            RuleOwner.DotnetFormatStyle, TokenDelta.None),
		new("IDE0250", "Make struct 'readonly'",                                           RuleOwner.Cleanup, TokenDelta.Inserted),
		new("IDE0251", "Make member 'readonly'",                                           RuleOwner.Cleanup, TokenDelta.Inserted),
	...
	/// <summary>The rules <c>curb cleanup</c> attempts.</summary>
	...
	/// <summary>True when <c>curb cleanup</c> attempts this rule.</summary>
