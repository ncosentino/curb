...
internal static partial class Printers
{
	...
	/// </code>
	///
	/// Curb asked the second question and so braced every body the author had put on its own line.
	...
	/// and run 2 unbrace it. Reported rather than silent; see CURB1004.
	/// </para>
	/// </remarks>
		...
		PrintContext context)
	{
		...
		// reproduces: if Curb does break after the `(`, the next run sees that break and keeps it.
	...
	public static void SwitchStatement(SwitchStatementSyntax node, PrintContext context)
	{
		...
		// dotnet format expands it as soon as either is off, and so does Curb.
	...
	public static void LocalFunctionStatement(LocalFunctionStatementSyntax node, PrintContext context)
	{
		...
		if (node.Body is not null)
		{
			if (!TryPrintExpressionBody(node.Body, context.Options.ExpressionBodiedLocalFunctions, context))
				...
				// function's brace, and `local_functions` alone moves nothing. Curb reading the
