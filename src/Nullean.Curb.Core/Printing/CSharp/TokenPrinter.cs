...
internal static class TokenPrinter
{
		...
		for (var i = skip; i < leading.Count; i++)
		{
			...
			switch (trivia.Kind())
			{
				...
				case SyntaxKind.MultiLineDocumentationCommentTrivia:
					...
					// indent. Curb used to normalise both, which pulled hand-aligned continuations
					// back to the left.
					//
	...
	private static bool AlignsUnderTrailingComment(SyntaxTrivia comment, PrintContext context)
	{
		...
		// A brace Curb is about to synthesise would land between the two comments.
	...
	/// used while every break Curb writes uses <c>end_of_line</c> — which left files with both, and
